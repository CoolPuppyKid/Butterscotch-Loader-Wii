...
static bool g_storageWritable = false;
...
static void openLogFile(const char* baseDir)
{
    ...
    if (WiiLog_open(logPath))
        ...
        g_storageWritable = false;
}

static void showStorageWriteWarning(void)
{
    const char* lines[] = {
...
int WiiGame_run(const WiiLaunchEntry* entry, bool audioEnabled)
{
    if (!entry || !entry->dataWinPath)
        return 1;

    const char* dataWinPath = entry->dataWinPath;
    loadingStep(4, 10, "OPEN DATA.WIN");
    FILE* dataWinProbe = fopen(dataWinPath, "rb");
    if (!dataWinProbe)
        fatalError("SELECTED DATA.WIN COULD NOT BE OPENED");
    fclose(dataWinProbe);
    ...
    g_storageWritable = probeWriteAccess(dataWinDir);
    if (g_storageWritable) {
        ensureDirectory(saveDir);
    } else {
        showStorageWriteWarning();
    }

    openLogFile(dataWinDir);
    fprintf(stderr, "FAT initialized (%s)\n", g_storageWritable ? "read/write" : "read-only");
    fprintf(stderr, "Selected game: %s (%s)\n",
            entry->title ? entry->title : "(unnamed)",
            dataWinPath);
    ...
    FileSystem* fileSystem = WiiFileSystem_createDefault(dataWin->gen8.displayName, dataWinPath, g_storageWritable);
    ...
    AudioSystem* audioSystem = audioEnabled
        ? (AudioSystem*) WiiAudioSystem_create()
        : (AudioSystem*) NoopAudioSystem_create();
